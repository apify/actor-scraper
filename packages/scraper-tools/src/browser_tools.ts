...
>(page: Page, instance: Instance, properties: readonly Keys[], getters: readonly PropertyKey[] = []) {
    const promises = properties
        .map((prop) => {
            ...
            if (!descriptor) {
                throw new Error(
                    `Cannot create a browser handle for property: ${prop} on object ${instance}. No such property descriptor.`,
                );
            }
            if (descriptor.value) {
                return {
            ...
            throw new Error(
                `Cannot create a browser handle for property: ${prop} on object ${instance}. No getter or value for descriptor.`,
            );
        })
...
export async function saveSnapshot({ page, body, contentType, json }: SnapshotOptions) {
    ...
    if (now - lastSnapshotTimestamp < SNAPSHOT.TIMEOUT_SECS * 1000) {
        log.warning(
            'Aborting saveSnapshot(). It can only be invoked once ' +
    ...
    if (json) {
        ...
        await Promise.all([
            Actor.setValue(SNAPSHOT.KEYS.BODY, html, {
                contentType: 'text/html',
            }),
            Actor.setValue(SNAPSHOT.KEYS.SCREENSHOT, screenshot, {
